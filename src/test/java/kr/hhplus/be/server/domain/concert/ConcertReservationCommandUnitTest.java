...
public class ConcertReservationCommandUnitTest {
	...
	void 유효한_입력값으로_객체_생성_성공() {
		...
		assertThatNoException()
			.isThrownBy(() -> new ConcertReservationCommand(concertScheduleId, concertScheduleDate, userId));
	}
	...
	void 콘서트_스케줄_ID가_0이하면_예외_발생() {
		...
		// when & then
		assertThatThrownBy(() -> new ConcertReservationCommand(invalidConcertScheduleId, concertScheduleDate, userId))
	...
	void 콘서트_날짜가_과거면_예외_발생() {
		...
		// when & then
		assertThatThrownBy(() -> new ConcertReservationCommand(concertScheduleId, pastDate, userId))
			.isInstanceOf(CustomException.class)
			.hasMessageContaining(CustomErrorCode.INVALID_DATE.getMessage());
	}

	@Test
	void 사용자_ID가_0이면_예외_발생() {
		// given
		long concertScheduleId = 1L;
		LocalDate concertScheduleDate = LocalDate.now().plusDays(7);
		...
		// when & then
		assertThatThrownBy(() -> new ConcertReservationCommand(concertScheduleId, concertScheduleDate, invalidUserId))
			.isInstanceOf(CustomException.class)
			.hasMessageContaining(CustomErrorCode.INVALID_USER_ID.getMessage());
	}

	@Test
	void 사용자_ID가_음수면_예외_발생() {
		// given
		long concertScheduleId = 1L;
		LocalDate concertScheduleDate = LocalDate.now().plusDays(7);
