...
class BalanceUnitTest {
	...
	void 최대포인트를_초과하면_포인트_충전에_실패한다() {
		...
		Balance balance = Balance.builder()
			.id(balanceId)
			.moneyVO(MoneyVO.create(BigDecimal.valueOf(100000)))
			.build();
	...
	void 최대포인트를_초과하지_않는다면_포인트_충전에_성공한다() {
		...
		Balance balance = Balance.builder()
			.id(balanceId)
			.moneyVO(MoneyVO.create(BigDecimal.valueOf(1000)))
			.build();
		//act
	...
	void 새로운_밸런스를_객체를_생성하며_유저와_연결한다() {
		// given
		long userId = 1L;
		MoneyVO moneyVO = MoneyVO.create(BigDecimal.valueOf(100000));

		//act
		Balance createBalance = Balance.create(moneyVO, LocalDateTime.now(), userId);
	...
	void 보유한_포인트_이상_사용시_예외처리() {
		// arrange
		long balanceId = 1L;
		MoneyVO moneyVO = MoneyVO.create(BigDecimal.valueOf(10000));
		BigDecimal usePoint = BigDecimal.valueOf(100000);

		Balance balance = Balance.create(moneyVO, LocalDateTime.now(), balanceId);
	...
	void pointVO가_null일때_예외발생() {
		...
		// act & assert
		assertThatThrownBy(() -> Balance.create(moneyVO, LocalDateTime.now(), userId))
	...
	void createdAt이_null이면_현재시간으로_설정된다() {
		...
		// act
		Balance balance = Balance.create(moneyVO, LocalDateTime.now(), userId);
	...
	void userId가_0이면_예외발생() {
		...
		// act & assert
		assertThatThrownBy(() -> Balance.create(moneyVO, LocalDateTime.now(), 0))
			.isInstanceOf(CustomException.class)
