...
class PaymentCommandUnitTest {
	...
	void 유효한_파라미터로_생성_성공() {
		// act
		PaymentCommand command = new PaymentCommand(1L, new BigDecimal("10000"));
	...
	void 좌석ID가_0이하면_예외발생() {
		// act, assert
		assertThatThrownBy(() -> new PaymentCommand(0L, new BigDecimal("10000")))
	...
	void 금액이_음수면_예외발생() {
		// act, assert
		assertThatThrownBy(() -> new PaymentCommand(1L, new BigDecimal("-1")))
			.isInstanceOf(CustomException.class)
