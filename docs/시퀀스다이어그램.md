...
sequenceDiagram
    ...
    %% Service ->> FQ : 대기열 순서 조회 요청
        ...
        alt 토큰이 대기상태 일 경우
            ...
            alt : 대기 순위가 1순위가 되었다면
                Service ->> Service: 토큰의 상태를 "활성(ACTIVATE)" 상태로 변경
                Service ->> TR: 토큰의 상태 저장 요청
                TR -->> Service: 토큰의 상태 저장 완료
            end
...
sequenceDiagram
    ...
    participant CS as  ConsertScheduleRepository 
    participant CSA as SeatRepository
...
sequenceDiagram
    ...
    participant SS as ReservationService
